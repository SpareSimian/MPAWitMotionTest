...
static DebugLog    p_WitDebugLogFunc = NULL;
...
#define FuncW 0x06 // MODBUS Write Single Register
#define FuncR 0x03 // MODBUS Read Holding Registers
...
static void WitDebugLog(const char *msg)
{
   if (!p_WitDebugLogFunc)
      return;
   p_WitDebugLogFunc(msg);
}
...
static int checkModbusCRC()
{
   const uint16_t usTemp = ((uint16_t)s_ucWitDataBuff[s_uiWitDataCnt-2] << 8) | s_ucWitDataBuff[s_uiWitDataCnt-1];
   const uint16_t usCRC16 = __CRC16(s_ucWitDataBuff, s_uiWitDataCnt-2);
   return (usTemp == usCRC16)
         ? 2 // we're done!
         : 0; // bad packet
}

static int isValidModbusSoFar()
{
   uint8_t ucBytes;
   uint8_t exception;

   if (0 == s_uiWitDataCnt)
      return 1; // empty packet is valid
   if (s_ucAddr != s_ucWitDataBuff[0])
      return 0; // incorrect address is invalid
   if (s_uiWitDataCnt > 1)
   {
      // check for valid function code, 0x80 means error response
      switch (s_ucWitDataBuff[1])
      {
      case FuncR:
      case FuncW:
      case 0x80 | FuncR:
   ...
   {
   case FuncR:
      // response from Read Holding Registers is a 1-byte byte
      // count, n 2-byte register values, and a 2-byte checksum
      ucBytes = s_ucWitDataBuff[2];
      if (s_uiWitDataCnt < (5u + ucBytes))
      {
         // CRC hasn't arrived yet
         // if no room for CRC, maybe size byte is garbage
         return (WIT_DATA_BUFF_SIZE == s_uiWitDataCnt) ? 0 : 1;
      }
      return checkModbusCRC();

   case FuncW:
      // Write Single Register has fixed length response:
      // address, function, 4 data bytes, CRC
      if (s_uiWitDataCnt < 8u)
         return 1; // no CRC yet
      return checkModbusCRC();
      
   case 0x80 | FuncR:
   case 0x80 | FuncW:
      // error response: address, function with 0x80, exception byte, CRC
      exception = s_ucWitDataBuff[2];
      if (s_uiWitDataCnt < 5)
         return 1; // no CRC yet
      return checkModbusCRC();

   default:
      return 0; // shouldn't happen, checked earlier
   }
...
void WitSerialDataIn(uint8_t ucData)
{
    ...
    //WitDebugLog("WitSerialDataIn");
    if(p_WitRegUpdateCbFunc == NULL)return ;
    if(s_uiWitDataCnt == (WIT_DATA_BUFF_SIZE - 1))
    {
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_MODBUS:
           ...
           {
              if (FuncR == s_ucWitDataBuff[1])
              {
                 // Read Single Register
                 usTemp = s_ucWitDataBuff[2] >> 1; // register count is 1/2 byte count
                 for(i = 0; i < usTemp; i++)
                 {
                 ...
                 //WitDebugLog("Wit Modbus read packet processed");
              }
              else
                 WitDebugLog("Wit Modbus non-read packet processed");
              // done with packet, get ready for next one
              s_uiWitDataCnt = 0;
           }
           break; 
...
int32_t WitDebugRegister(DebugLog log_func)
{
   if(!log_func)return WIT_HAL_INVAL;
   p_WitDebugLogFunc = log_func;
   return WIT_HAL_OK;
}
